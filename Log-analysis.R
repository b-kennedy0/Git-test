...
q <- ggplot(data, aes(date,time_since)) + geom_point() + geom_line() +
  geom_hline(yintercept=mean_time_since, linetype="dashed", color = "red") + labs(x = "Date", y = "Time Since")
q
...
a <- ggplot(freq, aes(date_only,n)) + geom_point() +geom_line() + 
              geom_hline(yintercept = mean_freq, linetype="dashed", color="red") + labs(x = "Date", y = "Number of times")
a
